module Halogen.Svg.Attributes.Path
  ...
  , CommandPositionReference(Abs, Rel)
  , CommandArcChoice(Arc0, Arc1)
  , CommandSweepChoice(Sweep0, Sweep1)
  , m, l, h, v, c, s, q, t, a, z
  ) where
...
c :: CommandPositionReference -> Number -> Number -> Number -> Number -> Number
  -> Number -> PathCommand
...
a :: CommandPositionReference -> Number -> Number -> Number -> CommandArcChoice
  -> CommandSweepChoice -> Number -> Number -> PathCommand
...
-- Functions below for internal use only. Do not export. -----------------------
